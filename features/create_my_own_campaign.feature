Feature: Create my own campaign
  ...
  @omniauth_test @bitly
  Scenario: when I create a new organization before create the new campaign
    Given I'm logged in
    And I'm in the new campaign page
    And I click "crie a sua organização clicando aqui!"
    And I fill "Nome" with "Meu Rio"
    And I attach an image to "Avatar"
    When I press "Cadastrar organização"
    Then I should be in the new campaign page
    And I should not see "Você representa alguma organização? Então crie a sua organização clicando aqui!"
    Given I fill "Qual é o título da sua campanha?" with "Evitar que desapareçam com a praça Nossa Senhora da Paz"
    And I fill "Por que essa campanha é importante pro Rio?" with "A praça é um patrimônio histórico e existem outras soluções para o metro que tomará o seu lugar."
    And I fill "Texto do email que será enviado para os alvos da campanha" with "A praça é um patrimônio histórico e existem outras soluções para o metro que tomará "
    And I fill "Texto da mensagem que será enviado no mural das páginas dos alvos da campanha" with "A praça é um patrimônio histórico e existem outras soluções para o "
    And I fill "Texto do tweet que será enviado, mencionando os alvos da campanha" with "A praça é um patrimônio histórico e existem outras soluções para o metro que "
    And I attach an image to "Imagem de campanha"
    And I select "Educação" for "Qual o tema da sua campanha?"
    And I check "Meu Rio"
    When I press "Enviar campanha para moderação"
